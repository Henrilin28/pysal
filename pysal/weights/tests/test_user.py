...
class Testuser(unittest.TestCase):
    ...
    def test_queen_from_shapefile(self):
        self.assertAlmostEqual(self.wq.pct_nonzero, 0.098292378175760101)

    def test_rook_from_shapefile(self):
        self.assertAlmostEqual(self.wr.pct_nonzero, 0.083298625572678045)
    ...
    def test_knnW_from_array(self):
        ...
        self.assertAlmostEqual(wnn2.pct_nonzero, 0.080000000000000002)
        self.assertAlmostEqual(wnn4.pct_nonzero, 0.16)
        wnn4 = pysal.knnW_from_array(data, k=4)
        self.assertEqual(set(wnn4.neighbors[0]), set([1, 5, 6, 2]))
        wnn3e = pysal.knnW(data, p=2, k=3)
        self.assertEqual(set(wnn3e.neighbors[0]),set([1, 5, 6]))
        wnn3m = pysal.knnW(data, p=1, k=3)
        self.assertEqual(set(wnn3m.neighbors[0]), set([1, 5, 2]))

    def test_knnW_from_shapefile(self):
        wc = pysal.knnW_from_shapefile(pysal.examples.get_path("columbus.shp"))
        self.assertAlmostEqual(wc.pct_nonzero, 0.040816326530612242)
        ...
        self.assertAlmostEqual(w.pct_nonzero, 0.011904761904761904)
        w1 = pysal.knnW_from_shapefile(
            pysal.examples.get_path('juvenile.shp'), k=1)
        self.assertAlmostEqual(w1.pct_nonzero, 0.0059523809523809521)
    ...
    def test_threshold_binaryW_from_array(self):
        ...
        self.assertEqual(w.weights, {0: [1, 1], 1: [1, 1], 2: [],
                                      3: [1, 1], 4: [1], 5: [1]})
        self.assertEqual(w.neighbors, {0: [1, 3], 1: [0, 3], 2: [
        ], 3: [1, 0], 4: [5], 5: [4]})
    ...
    def test_kernelW(self):
        ...
        self.assertEqual(kw.weights[0], [1.0, 0.50000004999999503,
                                          0.44098306152674649])
        self.assertEqual(kw.neighbors[0], [0, 1, 3])
